...
void merkle_element_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void merkle_element_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_size_of_witness() {return 9256;}
...
ctx->componentMemory[coffset].templateName = "merkle_element";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 7; i++) {
std::string new_cmp_name = "parents"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
merkle_element_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
}
}
...
if (!Fr_isTrue(&circuitConstants[3])) std::cout << "Failed assert in template " << myTemplateName << " line 53. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[15]); // line circom 57
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[3];
// load src
Fr_add(&expaux[0],&lvar[3],&circuitConstants[3]); // line circom 57
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[15]); // line circom 57
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[9]); // line circom 64
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[3])) + 0);
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * (Fr_toInt(&lvar[3]) * 2)) + 1)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
merkle_element_2_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * ((Fr_toInt(&lvar[3]) * 2) + 1)) + 1)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
merkle_element_2_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[3];
// load src
Fr_add(&expaux[0],&lvar[3],&circuitConstants[3]); // line circom 64
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[9]); // line circom 64
...
Fr_geq(&expaux[0],&lvar[4],&circuitConstants[3]); // line circom 79
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[6];
// load src
...
Fr_lt(&expaux[0],&lvar[6],&lvar[4]); // line circom 82
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[7];
// load src
Fr_add(&expaux[0],&lvar[5],&lvar[6]); // line circom 85
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[8];
// load src
Fr_mul(&expaux[3],&lvar[6],&circuitConstants[5]); // line circom 86
Fr_add(&expaux[1],&lvar[5],&expaux[3]); // line circom 86
Fr_mul(&expaux[2],&lvar[4],&circuitConstants[5]); // line circom 86
Fr_sub(&expaux[0],&expaux[1],&expaux[2]); // line circom 86
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[9];
// load src
Fr_mul(&expaux[4],&lvar[6],&circuitConstants[5]); // line circom 87
Fr_add(&expaux[2],&lvar[5],&expaux[4]); // line circom 87
Fr_add(&expaux[1],&expaux[2],&circuitConstants[3]); // line circom 87
Fr_mul(&expaux[2],&lvar[4],&circuitConstants[5]); // line circom 87
Fr_sub(&expaux[0],&expaux[1],&expaux[2]); // line circom 87
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
PFrElement aux_dest = &lvar[6];
// load src
Fr_add(&expaux[0],&lvar[6],&circuitConstants[3]); // line circom 82
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[6],&lvar[4]); // line circom 82
...
PFrElement aux_dest = &lvar[5];
// load src
Fr_add(&expaux[0],&lvar[5],&lvar[4]); // line circom 95
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[4];
// load src
Fr_div(&expaux[0],&lvar[4],&circuitConstants[5]); // line circom 98
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_geq(&expaux[0],&lvar[4],&circuitConstants[3]); // line circom 79
...
merkle_root_3_create(1,0,ctx,"main",0);
merkle_root_3_run(0,ctx);
}
